...
lista <- data.frame(file=paste0('data/',list.files('data')[25:31]),
                    ano=2016:2022)
...
db$me90 <- db$se.VD4022*1.65
db$me95 <- db$se.VD4022*1.96
db$me99 <- db$se.VD4022*2.58
db$ed <- ifelse (db$ano==2023, "1", '5')

db %>%
  filter (Capital=='Município de Recife (PE)') %>%
  ggplot (aes(x=ano)) +
  geom_ribbon(aes(ymin=VD4022-me99, ymax=VD4022+me99), fill='darkgrey') +
  geom_ribbon(aes(ymin=VD4022-me95, ymax=VD4022+me95), fill='lightgreen', alpha=1) +
  geom_ribbon(aes(ymin=VD4022-me90, ymax=VD4022+me90), fill='lightblue', alpha=1) +
  geom_line(aes(y=VD4022)) +
  geom_point(aes(y=VD4022, color=ed)) +
  geom_text (aes(y=VD4022,
                 label=paste0(round(VD4022,3),
  ...
  labs (title='Índice de Gini para o Recife com Margens de Erro',
        x=NULL,
        y=NULL,
        color='Entrevista\nmáxima',
        #caption='Índice de Gini para o Recife calculado a partir da variável VD4022 da PNAD Contínua (IBGE). Em azul, a margem de erro para 90% de confiança; em verde, a margem de erro para 95% de confiança; em cinza, a margem de erro para 99% de confiança. O IBGE ainda não disponibilizou os dados de todas as entrevistas para 2023, de forma que foram utilizados os dados acumulados da 1ª entrevista para este ano e da 5ª entrevista para os demais.'
        )

ggsave ('gini_me.png', width=10, height = 5)
...
db %>%
  ...
  theme_classic() +
  theme (legend.position = 'bottom') +
  labs (title='Índice de Gini para o Recife',
        x=NULL,
        y=NULL,
        color='Entrevista\nmáxima')

ggsave ('gini_linha.png', width=10, height = 5)
